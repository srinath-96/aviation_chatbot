...
def create_interface():
    ...
    with gr.Blocks(theme=theme) as interface:
        ...
        with gr.Row():
            chatbot_ui = gr.Chatbot(elem_id="chat_window", height=600)
        
        # Input area at the bottom: text input and file upload
        with gr.Row():
        ...
        def respond(message: str, history: list):
            try:
                bot_response = chatbot.generate_response(message, history)
                # Append a new tuple (user_message, bot_response) to the history
        ...
        # Link event handler to components
        file_input.change(
